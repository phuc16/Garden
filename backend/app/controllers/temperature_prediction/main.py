...
def main():
    ...
    timeGap = arguments['time_gap']

    df_temperature = pd.read_csv(
        arguments['file_temperature'], index_col=0, parse_dates=True)
    df_target_temperature = pd.read_csv(
        arguments['file_target_temperature'], index_col=0, parse_dates=True)
    df_valve = pd.read_csv(
        arguments['file_valve_level'], index_col=0, parse_dates=True)
    ...
    df_combined_resampled = df_combined.resample(pd.Timedelta(
        minutes=timeGap), label='right').mean().fillna(method='ffill')
    ...
    current = start - pd.DateOffset(minutes=timeGap)

    while current < stop:
        predicted_temperature, predicted_valve_level = perform_processing(
            df_temperature.loc[(current - pd.DateOffset(days=7)):current],
            df_target_temperature.loc[(
                current - pd.DateOffset(days=7)):current],
            df_valve.loc[(current - pd.DateOffset(days=7)):current],
            arguments['serial_number']
        )
        current = current + pd.DateOffset(minutes=timeGap)

        df_combined_resampled.at[current,
                                 'predicted_temperature'] = predicted_temperature
        df_combined_resampled.at[current,
                                 'predicted_valve_level'] = predicted_valve_level
